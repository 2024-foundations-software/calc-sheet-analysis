...
describe("CalculatorInputProcessor", () => {
    ...
    */
    ...
    it("clears the formula and edit status when the all clear button is clicked", () => {
        ...
        expect(getByTestId("formula")).toHaveTextContent("123");
 
        fireEvent.click(getByText("All Clear"));
        expect(getByTestId("formula")).toHaveTextContent("");
        expect(getByTestId("status")).toHaveTextContent("Done");
    });
 
    it("restarts the machine when the restart button is clicked", () => {
        const { getByText, getByTestId } = render(
            <CalculatorInputProcessor machine={machine} />
        );
